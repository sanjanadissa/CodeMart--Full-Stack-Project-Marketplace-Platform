...
const EditProject = () => {
  ...
  const [existingProjectUrl, setExistingProjectUrl] = useState<string>("");
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      try {
        ...
        setFormData({
          Name: response.name || "",
          Category: mapEnumToCategory(response.category) || "",
        ...
        setExistingVideoUrl(response.videoUrl || "");
        setExistingProjectUrl(response.projectUrl || "");
      } catch (err: any) {
  ...
  const handleImageFiles = (files: FileList | null) => {
    ...
    const maxNewImages = Math.max(0, 5 - existingImages.length);
    const currentNewImages = imageFiles.length;
    const availableSlots = maxNewImages - currentNewImages;

    if (availableSlots <= 0) {
      toast.error(
        `Maximum 5 images allowed. You already have ${existingImages.length} existing image(s) and ${currentNewImages} new image(s).`
      );
      return;
    }

    const imagesToAdd = imageFilesArray.slice(0, availableSlots);
    setImageFiles((prev) => [...prev, ...imagesToAdd]);

    if (imageFilesArray.length > availableSlots) {
      toast.warning(
        `Only ${availableSlots} image(s) added. Maximum 5 images total (${existingImages.length} existing + ${availableSlots} new).`
      );
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      let zipUrl = existingProjectUrl;
      if (zipFiles.length > 0) {
        toast.info("Uploading project files...");
        const uploadedZipUrl = await uploadToSupabase(zipFiles[0], true);
        if (!uploadedZipUrl) {
      ...
      if (videoFile.length > 0) {
        ...
        const uploadedVideoUrl = await uploadToSupabase(videoFile[0]);
        if (!uploadedVideoUrl) {
              ...
              <p className="text-gray-600 mb-4">
                ...
                {existingImages.length + imageFiles.length < 5 && (
                  <span className="block text-sm text-gray-500 mt-1">
                    ({5 - (existingImages.length + imageFiles.length)} slot(s)
                    remaining)
                  </span>
                )}
                {existingImages.length + imageFiles.length >= 5 && (
            ...
            {existingVideoUrl && (
              <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                ...
                <video
                  ...
                  controls
                  className="w-full max-w-md h-48 rounded-lg mb-2"
                />
                <button
                  type="button"
